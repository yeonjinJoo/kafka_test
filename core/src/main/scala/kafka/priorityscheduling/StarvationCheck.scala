...
class StarvationCheck {
  ...
  private val stride: Array[Long] = Array(25L, 5L, 1L)

  // 각 큐는 연속적으로 요청 최대 ~개까지만 처리 할 수 있다. 큐 별로 정해진 값
  // 다른 큐 애들 중 작은 애 stride 기준으로 걔랑의 gap을 구하게 될 테니까 걔 stride * 내가 원하는 최대 연속 처리 갯수
  private val occupyThreshold: Array[Long] = Array(1L, 5L, 25L)
  ...
  def getMinPassQueueNum(sizes: Array[Int]): Int = {
    ...
    // 요청이 하나도 없는 경우가 아닐 때
    if (queueNum != 0) {
      if (!isPassFalledBehind(queueNum)) {
  ...
  private def getTimeOutRiskyQueueNum(headAgeMsForPriorityQueues: Array[Long],
    ...
    for (i <- 0 until 3) {
      ...
      if (headAge >= 0 && timeout >= 0) {
        // timeout 시간의 70% 이상으로 head가 오래된 경우
        if (headAge >= ((timeout * 7L) / 10L) && headAge > maxTimeElapsed) {
          queueNum = i + 1
  ...
  def scheduleAndPick(rc: RequestChannel,
                      ...
                      timeoutMsForPriorityQueues: Array[Long]): (Int, Boolean) = {
    var chosen = 0
    var isUselss = false
    ...
    if (s1 == 0 && s2 == 0 && s3 == 0) return (0, false)

    val sizes: Array[Int] = Array(s1, s2, s3)

    // 0. 현재 요청이 들어있는 Queue가 1개 뿐이라면 바로 해당 Queue 처리.
    // priority queue 하나에만 request가 있을 때, 혼자 있을 때 pass 너무 커지는 걸 막기 위해 본인 제외 나머지 큐가 모두 비어있다면 pass 증가 x
    val notEmptyQs = sizes.zipWithIndex.collect { case (sz, idx) if sz > 0 => idx + 1 }
    if (notEmptyQs.length == 1) {
      chosen = notEmptyQs(0)
      isUselss = true
      return (chosen, isUselss) // 1개만 있어서 바로 처리되는 경우는 테스트 통계에 유효한 자료 x
    }
    ...
    if (chosen != 0) {
      if (reqLog.isTraceEnabled) {
        reqLog.trace(s"TimeoutRisk - [PICK] P$chosen; size=${sizes(chosen) - 1} | sizes=[P1=${sizes(0)}, P2=${sizes(1)}, P3=${sizes(2)}], pass=[P1=${pass(0)}, P2=${pass(1)}, P3=${pass(2)}]")
      }
      return (chosen, isUselss)
    }
