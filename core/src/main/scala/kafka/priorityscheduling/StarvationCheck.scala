...
class StarvationCheck {
  ...
  private val starvationThreshold = 0 // 설정 필요 - 그냥 값임. 읽어오기만 o.
  ...
  def increaseStarvationCount(rc: RequestChannel): Unit = {
    lock.lock()
    try {
      if (rc.getRequestQueueP1Size() != 0) {
        starvationCount[0] += 1
      }

      if (rc.getRequestQueueP2Size() != 0) {
        starvationCount[1] += 1
      }

      if (rc.getRequestQueueP3Size() != 0) {
        starvationCount[2] += 1
      }
    } finally {
      lock.unlock()
  ...
  def starvationBoosting(): Unit = {
    lock.lock()
    try{
      var minPassValue = getMinPassValue()
      for(int i = 0; i < 3; i++){
  ...
  def getMinPassQueueNum(rc: RequestChannel): Int = {
    ...
    try{
      ...
      if(rc.getRequestQueueP3Size() != 0 && pass[3] < minPassValue){
        minPassValue = pass[2]
        queueNum = 3
      }
  ...
  def getMinPassValue(): Long = {
    ...
    if (min == Long.MaxValue) 0L else minPassValue
  }
